...
class ChatsController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating, UISearchBarDelegate, UISearchControllerDelegate {
    ...
    func numberOfSections(in _: UITableView) -> Int {
        let count = deviceUser.chatrooms?.chats.count ?? 0
        return count + 1
    }

    func tableView(_: UITableView, heightForHeaderInSection _: Int) -> CGFloat {
        if deviceUser.chatrooms?.chats.count != 0 {
    ...
    func tableView(_: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if count == 0 {
            ...
            if let chatroom = deviceUser.chatrooms?.chats[indexPath.section] {
    ...
    func tableView(_: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        if section < deviceUser.chatrooms?.chats.count ?? 0 {
            return 0
        } else {
            return 0.01
        }
    ...
    func tableView(_: UITableView, canEditRowAt _: IndexPath) -> Bool {
        if deviceUser.chatrooms?.chats.count ?? 0 > 0 {
            return true
        } else {
            return false
        }
    ...
    func updateSearchResults(for searchController: UISearchController) {
        if let searchString = searchController.searchBar.text {
            if searchString.characters.count > 0 && deviceUser.chatrooms != nil {
                filteredChatrooms = deviceUser.chatrooms!.chats.filter({ (_) -> Bool in
                    false // activity.name!.contains(searchString)
                })
            } else {
                filteredChatrooms = deviceUser.chatrooms?.chats ?? []
            }
        }
...
extension ChatsController: TSWThemeSupportedProtocol {
