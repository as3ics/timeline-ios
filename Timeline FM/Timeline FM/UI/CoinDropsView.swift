...
open class DropsView: UIView {
    ...
    public init(frame: CGRect, direction: DropDirection = .up, dropNum: Int = 10, color: UIColor = UIColor.blue.withAlphaComponent(0.7), minDropSize: CGFloat = 4, maxDropSize: CGFloat = 10, minLength: CGFloat = 0, maxLength: CGFloat = 100, minDuration: TimeInterval = 4, maxDuration: TimeInterval = 12, image: UIImage? = nil)  {
    ...
    fileprivate func randomWaterdrop(config: ViewConfig, direction: DropDirection = .up) {
        ...
        var waterdrop : UIImageView? = UIImageView()
        waterdrop?.frame = CGRect(x: randomX, y: positionY, width: randomSize, height: randomSize)
        waterdrop?.backgroundColor = self.image != nil ? UIColor.clear : self.color
        waterdrop?.image = self.image != nil ? self.image : nil
        waterdrop?.tintColor = self.color
        waterdrop?.contentMode = .scaleAspectFit
