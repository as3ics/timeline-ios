...
extension CLLocationCoordinate2D {
    ...
    func googleMaps() {
        if UIApplication.shared.canOpenURL(URL(string: "comgooglemaps://")!) {
            let url = NSURL(string: "comgooglemaps://?saddr=&daddr=\(Float(latitude)),\(Float(longitude))&directionsmode=driving")! as URL
            UIApplication.shared.open(url, options: EMPTY_JSON, completionHandler: nil)
        } else {
            DispatchQueue.main.async(execute: {
                let message = "Google Maps is not installed on this device. Use Apple Maps?"
                let alertController = UIAlertController(title: App.shared.appName, message: message, preferredStyle: .alert)
                alertController.addAction(UIAlertAction(title: "Yes", style: .default, handler: { _ in
                    self.appleMaps(name: nil)
                }))
    ...
    func offset(metersX: Double, metersY: Double) -> CLLocationCoordinate2D {
        return CLLocationCoordinate2D(latitude: self.latitude + (metersX / meterPerLat(self)), longitude:self.longitude + (metersY / metersPerLon(self)))
    }
}
...
extension UIViewController {
    ...
    func styleSearchBar(_ searchController: UISearchController) {
        ...
        searchBar.setSearchFieldBackgroundImage(Assets.shared.clear, for: UIControlState.normal)
        ...
        textFieldInsideUISearchBar?.textColor = Theme.shared.active.primaryFontColor
    ...
    func styleSwitch(_ swtch: UISwitch) {
        swtch.thumbTintColor = Theme.shared.active.secondaryBackgroundColor
        swtch.tintColor = Theme.shared.active.secondaryBackgroundColor
    }
...
extension String {
    ...
    func user() -> User? {
        guard DeviceUser.shared.user?.id != self else {
            return DeviceUser.shared.user
        }
        
        let matches = Users.shared.items.filter { (user) -> Bool in
            return user.id == self
        }
    ...
    func activity() -> Activity? {
        let matches = Activities.shared.items.filter { (activity) -> Bool in
            return activity.id == self
        }
    ...
    func location() -> Location? {
        let matches = Locations.shared.items.filter { (location) -> Bool in
            return location.id == self
        }
...
extension FlatButton {
    ...
    func style(_ color: UIColor, image: UIImage?, title: String? = nil, inset: CGFloat = 7.5) {
...
extension UITextField {
    func autoResize() {
        adjustsFontSizeToFitWidth = true
        //minimumFontSize = font!.pointSize * 0.75
    }
...
extension Date {
    ...
    func lean() -> Date {
        ...
        let components = DateComponents(calendar: calendar, timeZone: TimeZone.current, era: nil, year: year, month: month, day: day, hour: nil, minute: nil, second: nil, nanosecond: nil, weekday: nil, weekdayOrdinal: nil, quarter: nil, weekOfMonth: nil, weekOfYear: nil, yearForWeekOfYear: nil)
        
        return calendar.date(from: components)!
    }
    
    mutating func incrementDay() {
        self = self.addingTimeInterval(TimeInterval(60.0 * 60.0 * 24.0))
    }
